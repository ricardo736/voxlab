...
import { Routine, Theme } from '../types';
import { ROUTINES, EXERCISES } from '../constants';
import { useTranslation } from '../hooks/useTranslation';
import { getExercisePattern, getExerciseDurations, getExerciseId } from '../exerciseUtils';
...
const RoutineCard: React.FC<{
    ...
    const exercisesById = useMemo(() => new Map(EXERCISES.map(ex => [getExerciseId(ex), ex])), []);
    ...
    const totalDurationMs = useMemo(() => {
        return routine.exerciseIds.reduce((sum, id) => {
            ...
            for (let t = 0; t <= MAX_TRANSPOSE; t++) {
                ...
                exerciseTime += totalDuration * 1000 + 500;
            }
    ...
    const TimeIcon = () => <svg className="w-5 h-5 mr-1 text-sky-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>;
    ...
    const handleCardClick = () => {
        ...
        // If card is not centered (more than 50px from center), scroll to center
        if (distanceFromCenter > 50) {
            cardRef.current.scrollIntoView({ behavior: 'smooth', inline: 'center', block: 'nearest' });
        } else {
            // Card is centered, allow flip
            setIsFlipped(!isFlipped);
        }
    };

    return (
        <div ref={cardRef} className="w-full h-full cursor-pointer" style={{ perspective: '1000px' }}>
            <div
            ...
            >
                ...
                >
                    ...
                    <div>
                        <div className="flex items-center text-sm text-slate-600 dark:text-slate-300 bg-slate-100/70 dark:bg-slate-900/50 rounded-full px-3 py-1 mb-3 w-fit">
                            <TimeIcon />
                            <span className="font-semibold">~{totalDurationMinutes} min</span>
                        </div>
                        <h3 className={`text-2xl font-extrabold bg-clip-text text-transparent bg-gradient-to-br ${currentTheme.gradientText.from} ${currentTheme.gradientText.to} ${currentTheme.gradientText.darkFrom} ${currentTheme.gradientText.darkTo} mb-2`}>{t(routine.name)}</h3>
                        <p className="text-slate-600 dark:text-slate-400 text-sm">{t(routine.description)}</p>
                    </div>
                    <button
                ...
                <div className={`select-none absolute w-full h-full p-6 flex flex-col justify-between bg-white/80 dark:bg-slate-800/60 backdrop-blur-3xl rounded-3xl shadow-lg border border-white/50 dark:border-slate-700/50 overflow-hidden ${!isFlipped ? 'pointer-events-none' : ''}`} style={{ backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}>
                    <div className="flex-grow flex flex-col min-h-0">
                        ...
                        <h4 className={`text-lg font-bold bg-clip-text text-transparent bg-gradient-to-br ${currentTheme.gradientText.from} ${currentTheme.gradientText.to} ${currentTheme.gradientText.darkFrom} ${currentTheme.gradientText.darkTo} mb-2`}>{t('exercises')}</h4>
                        <div className="flex-grow overflow-y-auto scrollbar-hide">
                            <div className="flex flex-wrap gap-2">
                                {routine.exerciseIds.map(id => {
                                    const ex = exercisesById.get(id);
                                    return ex ? (
                                        <span key={id} className="text-slate-700 dark:text-slate-300 text-xs font-medium bg-slate-100/80 dark:bg-slate-900/50 rounded-full px-3 py-1 border border-slate-200/80 dark:border-slate-700/80">{t(ex.name)}</span>
                                    ) : null;
                                })}
                            </div>
                        </div>
                    </div>
                    <ThemedButton onClick={(e) => { e.stopPropagation(); onStart(routine); }} theme={currentTheme} className="w-full mt-4">
                        {t('startRoutine')}
                    </ThemedButton>
                </div>
            </div>
...
const RoutineView: React.FC<RoutineViewProps> = ({ onStartRoutine, currentTheme, favoriteRoutineIds, onToggleFavorite }) => {
    ...
    return (
        <section className="flex-grow flex flex-col items-center justify-center w-full -mt-24 md:mt-0">
            <div className="relative w-full h-[450px] md:h-[400px]" style={{ perspective: '2000px' }}>
                <button
                ...
                >
                    <svg className="w-6 h-6 text-slate-700 dark:text-slate-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
                </button>
                <div
                ...
                >
                    <svg className="w-6 h-6 text-slate-700 dark:text-slate-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                </button>
            </div>
            ...
            <p className="text-center text-xs text-slate-400 dark:text-slate-500 mt-2 md:hidden">
                {t('swipeRoutines')}
            </p>
        </section>
    );
