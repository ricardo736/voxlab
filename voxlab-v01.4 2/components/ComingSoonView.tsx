...
interface ComingSoonViewProps {
    title: string;
    description: string;
    currentTheme: { primary: string, gradientText: { from: string, to: string, darkFrom: string, darkTo: string } };
    isStudies?: boolean;
}
...
const ComingSoonView: React.FC<ComingSoonViewProps> = ({ title, description, currentTheme, isStudies }) => {
    ...
    if (isStudies) {
        ...
        return (
            <section className="flex-grow flex flex-col justify-center animate-fade-in w-full pt-8">
    ...
    return (
        <section className="flex-grow flex flex-col justify-center items-center w-full animate-fade-in pt-8">
            <div className="w-full max-w-md">
                 <ComingSoonCard title={title} description={description} theme={currentTheme}>
                    <div className="h-24 w-full flex items-center justify-center rounded-lg border-2 border-dashed border-slate-300 dark:border-slate-600 bg-slate-100/50 dark:bg-slate-800/50">
