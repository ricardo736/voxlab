...
const ExerciseGameViewALTWrapper = React.lazy(() => import('./components/ExerciseGameViewALTWrapper'));
const ErrorBoundary = React.lazy(() => import('./components/ErrorBoundary'));
const AIExerciseView = React.lazy(() => import('./components/AIExerciseView'));
const InputVolumeMeter = React.lazy(() => import('./components/InputVolumeMeter'));
const FloatingMenu = React.lazy(() => import('./components/FloatingMenu'));
const VocalRangeTestScreen = React.lazy(() => import('./components/VocalRangeTestScreen'));
const SettingsOverlay = React.lazy(() => import('./components/SettingsOverlay'));
import { useTheme } from './context/ThemeContext';
import { useAudio } from './context/AudioContext';
import { useAudioEngine } from './hooks/useAudioEngine';
import { useAudioPlayer } from './hooks/useAudioPlayer';
const RoutineView = React.lazy(() => import('./components/RoutineView'));
const ComingSoonView = React.lazy(() => import('./components/ComingSoonView'));
const VoxLabAIView = React.lazy(() => import('./components/AIStudioView'));
const TestModeView = React.lazy(() => import('./components/TestModeView'));
const FavoritesView = React.lazy(() => import('./components/FavoritesView'));
const InstrumentTuner = React.lazy(() => import('./components/InstrumentTuner'));
const ThemedButton = React.lazy(() => import('./components/ThemedButton'));
const OrbVisualizer = React.lazy(() => import('./components/OrbVisualizer'));
const FeedbackOverlay = React.lazy(() => import('./components/FeedbackOverlay'));
...
export default function App() {
    ...
    // micActive, userPitch, micGain, micStatus moved to useAudioEngine hook
    ...
    const { activeTheme, themeId, setThemeId, themeMode, setThemeMode } = useTheme();
    ...
    // Audio Context from Provider (still needed for some local logic if any)
    ...
    // Audio Engine Hook (Encapsulates all audio logic)
    const {
        micActive,
        setMicActive,
        userPitch,
    ...
    // Audio Player Hook (Handles samples and playback)
    const {
        activeInstrument,
        setActiveInstrument,
    ...
    // Theme effect moved to ThemeContext
    ...
    useEffect(() => {
        localStorage.setItem('micGain', JSON.stringify(gainValue));
    }, [gainValue]);
            ...
            >
                <div className="relative z-10 flex flex-col h-full">
                    ...
                    {isRangeTestActive && (
                        <React.Suspense fallback={
                            <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-white dark:bg-slate-900">
                                <div className="w-12 h-12 border-4 border-slate-200 border-t-violet-500 rounded-full animate-spin"></div>
                                <p className="mt-4 text-slate-500 font-medium">Loading Range Detector...</p>
                            </div>
                        }>
                            <VocalRangeTestScreen
                                onCancel={handleCancelRangeTest}
                                onComplete={handleCompleteRangeDetection}
                    ...
                    {uiView !== 'exercise' && !isRangeTestActive && (
                        <header
                            className="sticky top-0 z-40 flex items-end justify-between px-4 sm:px-6 lg:px-8 bg-white/20 dark:bg-slate-900/20 backdrop-blur-md"
                            style={{
                                paddingTop: 'max(1.5rem, calc(env(safe-area-inset-top) + 0.5rem))',
                                paddingBottom: '0.75rem'
                            }}
                        >
                            <h1 className={`text-4xl sm:text-5xl font-black bg-gradient-to-br ${activeTheme.gradientText.from} ${activeTheme.gradientText.to} ${activeTheme.gradientText.darkFrom} ${activeTheme.gradientText.darkTo} bg-clip-text text-transparent truncate pr-4`}>
                            ...
                            <div className="flex items-center gap-3 mb-1">
                                ...
                                {/* Removed InputVolumeMeter as requested */}
                            </div>
                        </header>
                    )}
                        ...
                        <React.Suspense fallback={
                            ...
                            {uiView === 'main' && (
                                <div key={activeView} className="flex-grow flex flex-col animate-fade-in">
                                    {/* Background decorative orbs for non-home views */}
                                    {activeView !== 'home' && activeView !== 'test' && (
                                        <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
                                            <React.Suspense fallback={null}>
                                                <OrbVisualizer micGain={micGain} visualizerTheme={activeTheme.visualizer} />
                                            </React.Suspense>
                                        </div>
                                    )}
